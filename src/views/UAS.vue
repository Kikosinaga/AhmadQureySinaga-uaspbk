    ...
    <div v-if="showEditModal" class="modal-overlay">
      <div class="modal-content">
        ...
        <div class="input-group">
          <label for="editMatkulUAS">Mata Kuliah:</label>
          <input id="editMatkulUAS" v-model="currentEditSoal.matkul" placeholder="Masukkan Mata Kuliah" class="edit-input" />
        </div>

        <div class="input-group">
          <label for="editDosenUAS">Dosen:</label>
          <input id="editDosenUAS" v-model="currentEditSoal.dosen" placeholder="Masukkan Nama Dosen" class="edit-input" />
        </div>

        <div class="input-group">
          <label for="editSemesterUAS">Semester:</label>
          <input id="editSemesterUAS" v-model="currentEditSoal.semester" placeholder="Masukkan Semester" class="edit-input" />
        </div>

        <div class="input-group">
          <label for="editTahunUAS">Tahun:</label>
          <input id="editTahunUAS" v-model="currentEditSoal.tahun" placeholder="Masukkan Tahun" class="edit-input" />
        </div>
...
import { useSoalStore } from '../store/soal' 
...
function handleOpenEditModal(soalToEdit) {
  ...
  console.log('Membuka modal edit untuk soal UAS:', currentEditSoal.value); 
}
...
.modal-overlay {
  ...
  width: 100vw;
  height: 100vh;
  background: rgba(0,0,0,0.5); /* Tetap gelap agar modal fokus, atau bisa rgba(pink_value, 0.3) */
  display: flex;
  align-items: center;
